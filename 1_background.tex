...
As stated in the introduction, one of the main design goals of the system is usability. This is accomplished in practice by building it
\js{bez ``it''}
on top of the most accessible and ubiquitous platform -- the web platform\cite{web_platform}\footnote{Also referred to as the \textit{open} web platform\cite{open_web_platform}}.
...
\js{Jakieś takie zagubione i samotne to zdanie}
...
is problematic when a single message takes long to execute. This
\js{Chcę zasugerować unikanie powtarzania ``this'' poprzez użycie ``the''. Nie zawsze się da, ale tutaj jak najbardziej tak.}
 problem is
...
\js{Musiałem wrócić do wcześniejszego zdania żeby przypomnieć sobie kim jest ten ``he''. Może lepiej ``the programmer''? Nawiasem mówiąc w literaturze dotyczącej PL jest zwyczaj używania ``she'', zamiast ``he''.}
...
% other disadvantages and features
% Unreal
